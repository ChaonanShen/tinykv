...
func TestRollback4C(t *testing.T) {
	...
	assert.Nil(t, resp.Error)
	assert.Nil(t, resp.RegionError)
	builder.assertLens(0, 0, 1) // 在default列族居然有了数据，奇怪奇怪
	builder.assert([]kv{
...
func TestCheckTxnStatusTtlExpired4C(t *testing.T) {
	...
	// lock.key=3 lock.kind=put
	// lock.Ts= "0, 0, 5, 0, 0, 0, 0, builder.ts()"
	// lock.Ttl="0, 0, 0, 0, 0, 0, 0, 8" -- 所以未过期
...
func TestScanDeleted4C(t *testing.T) {
	...
	// version=120
	// 120 150 199都看得见，而且150对应value换的
...
func TestScanAll4C(t *testing.T) {
	...
	assert.Nil(t, resp.RegionError)
	assert.Equal(t, 11, len(resp.Pairs)) // 前10个再加上150
...
func builderForScan(t *testing.T) *testBuilder {
	values := []kv{
		...
		{engine_util.CfWrite, []byte{150}, 99, []byte{1, 0, 0, 0, 0, 0, 0, 0, 80}},  // ts=99 put
		{engine_util.CfWrite, []byte{150}, 101, []byte{2, 0, 0, 0, 0, 0, 0, 0, 97}}, // ts=101 delete
		{engine_util.CfDefault, []byte{150}, 110, []byte{64}},
		{engine_util.CfWrite, []byte{150}, 116, []byte{1, 0, 0, 0, 0, 0, 0, 0, 110}}, //ts=116 put
	} // CfWrite第一位表示put/delete/rollback
	builder := newBuilder(t)
	builder.init(values)
	return &builder
}
